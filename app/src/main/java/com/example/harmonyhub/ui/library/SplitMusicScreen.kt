...
import com.example.demodowloadmusic.SplitUiState
import com.example.demodowloadmusic.SplitViewModel
...
fun SplitMusicScreen(
    ...
    viewModel: SplitViewModel = viewModel(factory = SplitViewModel.Factory2)
) {
    ...
    val state1 = viewModel.stateMusic1.collectAsStateWithLifecycle().value
    val state2 = viewModel.stateMusic2.collectAsStateWithLifecycle().value
    if (state1 == SplitUiState.Loading && state2 == SplitUiState.Loading) {
        LoadingScreen()
    } else if (state1 == SplitUiState.Error && state2 == SplitUiState.Error){
        ErrorScreen {
            viewModel.splitMusic1()
            viewModel.splitMusic2()
        }
    } else {
        Column(
            modifier = Modifier
                        ...
                        Row(
                            ...
                            Checkbox(
                                ...
                                onCheckedChange = { isChecked ->
                                    ...
                                    if (!isPlaying.value) {
                                        checkedStates.value =
                                            checkedStates.value.toMutableMap().apply {
                                                put(part, isChecked)
                                            }
                                    }
                                },
                                colors = CheckboxDefaults.colors(
                                    checkedColor = Color(0xFF00FAF2),
